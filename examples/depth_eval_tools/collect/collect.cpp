...
int main(int argc, char **argv)
{
    ...
    int startIndex = parser.get<int>("sf");
    ...
    num_views += startIndex;
    for (int view_idx = startIndex; view_idx < num_views; view_idx++)
    {
        std::cout << "----------  Ready to capture next frame. ---------- Press Enter. " << std::endl;
        getchar();
        for (uint32_t dev = 0; dev < device_count; ++dev)
        {
            k4a::device device = k4a::device::open(dev);
            std::string output_dev_dir = output_dir + "/" + std::to_string(dev);
            ...
            if (dump_calibration)
            {
                write_calibration_blob(device.get_raw_calibration(), output_dev_dir, "calibration_blob");
                write_opencv_calib(calibration.depth_camera_calibration,
                                   output_dev_dir,
                                   "cal_depth_mode" + std::to_string(mode));
                write_opencv_calib(calibration.color_camera_calibration, output_dev_dir, "cal_color");
            }
            ...
            if (dump_xytable)
            {
                ...
                write_xy_table(xy_table, output_dev_dir, "xy_table_mode" + std::to_string(mode));
            }

            k4a::capture capture;

            std::chrono::milliseconds timeout_msec(4000);
                ...
                if (dump_color)
                {
                    ...
                    if (color_img.is_valid())
                    {
                        ...
                        if (av > 0)
                        {
                            ...
                            cv::imwrite(output_dev_dir + "/" + tstr + "-color.png", M);
                        }
                        n_color++;
                    }
                }
                ...
                if (dump_depth)
                {
                    ...
                    if (depth_img.is_valid())
                    {
                        ...
                        if (av > 0)
                        {
                            ...
                            cv::imwrite(output_dev_dir + "/" + tstr + "-depth16.png", M);
                        }
                        n_depth++;
                    }
                }
                ...
                if (dump_ir)
                {
                    ...
                    if (ir_img.is_valid())
                    {
                        ...
                        if (av > 0)
                        {
                            ...
                            cv::imwrite(output_dev_dir + "/" + tstr + "-ir16.png", M);
                        }
                        n_ir++;
                    }
                }
            ...
            if (n_ir > 0 && av != 1)
            {
                ...
                cv::imwrite(output_dev_dir + "\\" + "ir8-" + std::to_string(view_idx) + ".png", ir8);
                cv::imwrite(output_dev_dir + "\\" + "ir16-" + std::to_string(view_idx) + ".png", ir16);
            }
            if (n_depth > 0 && av != 1)
            {
                mean_depth /= n_depth;
                ...
                cv::imwrite(output_dev_dir + "\\" + "depth16-" + std::to_string(view_idx) + ".png", depth16);
            }
            if (n_color > 1 && av != 1)
            {
                cv::Mat channels[3];
                ...
                cv::imwrite(output_dev_dir + "\\" + "color-" + std::to_string(view_idx) + ".png", color8);
            }
            device.stop_cameras();
            device.close();
        }
